...
async def get_shortlink(link, x=""):
    ...
    url = f'https://tnlinks.in/api'
    params = {'api': TNLINKS_API,
              'url': link,
              'alias': x
              }
    ...
    try:
        ...
        logger.error(e)
        links = f'https://tnlinks.in/st?api={TNLINKS_API}&url={link}'
...
async def replace_link(text, x=""):
    ...
    for link in links:
        ...
        tnlinks_url = await is_tnlinks_url(link)  

        if LINK_BYPASS and tnlinks_url or not tnlinks_url:
            # Bypass Tnlinks URL
            if LINK_BYPASS and tnlinks_url:
                try:
                    link = await tnlinks_bypass(link)
                except Exception as e:
                    logger.exception(e)
...
async def mdisk_tnlinks_convertor(text, alias=""):
    links = await replace_mdisk_link(text)
    links = await replace_link(links, x=alias)
    return links

####################  Mdisk and Tnlinks Reply Markup ####################

async def mdisk_tnlinks_convertor_reply_markup(text):
    links = await replace_mdisk_link(text)
...
# Incase tnlinks server fails, bot will return https://tnlinks.in/st?api={TNLINKS_API}&url={link} 
...
async def tnlinks_bypass_handler(text):
    ...
    for link in links:
        bypassed_link = await tnlinks_bypass(link)
                ...
                async with client.get(url, headers=h) as res:
                    ...
                    async with client.post(final_url, data=data, headers=h) as res:
                        ...
                        if res['status'] == 'success':
                            ...
                            raise Exception("Error while bypassing tnlinks {0}: {1}".format(url, res['message']))
            await client.close()

    except Exception as e:
        raise Exception("Error while bypassing tnlinks {0}: {1}".format(url, e))


async def is_tnlinks_url(url):
    domain = urlparse(url).netloc
    domain = url if "tnlinks.in" in domain else False
    return domain
...
async def update_stats(m:Message, method):
    ...
    tnlinks_links = await extract_link(message + reply_markup)
    total_links = len(tnlinks_links)

    await db.update_posts(1)

    if method == 'mdisk': tnlinks_links = []
    if method == 'tnlinks': mdisk_links = []

    await db.update_links(total_links, len(tnlinks_links), len(mdisk_links))
